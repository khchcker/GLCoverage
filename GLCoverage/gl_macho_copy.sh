...
function main {
    echo "\n"
    hecho "macho & gl_parse_profraw.sh & gitdiff copy begin"
    ...
    hecho "macho & gl_parse_profraw.sh & gitdiff copy end"
    echo "\n"
}
...
#    cp -r $CODESIGNING_FOLDER_PATH $machoFiles
    ...
    parseProfrawPath="$resulePath/gl_parse_profraw.sh"
    parseProfrawGatherPath="$resulePath/gl_parse_profraw_gather.sh"

    cp -r ${scripts}/gl_parse_profraw.sh $resulePath
    cp -r ${scripts}/gl_parse_profraw_gather.sh $resulePath
